...
export async function upsertFormHistory(row: FormHistoryRow): Promise<void> {
  await exec(
    `INSERT INTO form_history (date, output, saved_at, responses, schema_version_id)
     VALUES (?, ?, ?, ?, ?)
    ...
    [row.date, row.output, row.saved_at, row.responses, row.schema_version_id]
  )
}

export async function getFormHistory(): Promise<FormHistoryRow[]> {
  return toObjects(
    await query('SELECT date, output, saved_at, responses, schema_version_id FROM form_history ORDER BY date DESC')
  )
}
...
/** Fetches all non-empty 'happened' values from form_history for sentence analysis. */
export async function getFormHistoryHappenedTexts(): Promise<string[]> {
  const rows = toObjects<{ happened: string }>(
    await query(`SELECT json_extract(responses, '$.happened') AS happened
                 FROM form_history
                 WHERE responses IS NOT NULL AND json_extract(responses, '$.happened') IS NOT NULL`)
  )
